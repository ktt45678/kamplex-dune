...
import { AfterViewInit, ChangeDetectionStrategy, ChangeDetectorRef, Component, ViewContainerRef, ComponentRef, ElementRef, NgModule, NgZone, OnDestroy, Renderer2, Type, ViewChild, ViewEncapsulation, ViewRef } from '@angular/core';
...
@Component({
  ...
  template: `
      ...
      >
          ...
          >
              ...
              <p-dialog-header #titlebar *ngIf="!config.minimal && config.showHeader !== false"></p-dialog-header>
  ...
  styleUrls: ['../../../../../../node_modules/primeng/resources/components/dialog/dialog.css'],
  host: {
    class: 'p-element'
  }
})
export class DynamicDialogComponent implements AfterViewInit, OnDestroy {
  ...
  documentResizeListener!: ((this: Document, ev: MouseEvent) => any) | null;

  documentResizeEndListener!: ((this: Document, ev: MouseEvent) => any) | null;

  documentDragListener!: ((this: Document, ev: MouseEvent) => any) | null

  documentDragEndListener!: ((this: Document, ev: MouseEvent) => any) | null;
  ...
  loadChildComponent(componentType: Type<any>) {
    let viewContainerRef = this.insertionPoint?.viewContainerRef || this.viewContainerRef;
  ...
  moveOnTop() {
    if (this.config.autoZIndex !== false) {
      ...
      this.wrapper && (this.wrapper.style.zIndex = String(parseInt(this.container!.style.zIndex, 10) - 1));
    }
  }
  ...
  template: ` <div class="p-dialog-header" (mousedown)="initDrag($event)">
      ...
      <div class="p-dialog-header-icons">
          <button *ngIf="config.maximizable" type="button" class="p-dialog-header-icon p-dialog-header-maximize p-link" (click)="maximize()" (keydown.enter)="maximize()" tabindex="-1" pRipple>
              <span class="p-dialog-header-maximize-icon" [class]="maximized ? minimizeIcon : maximizeIcon"></span>
          </button>
...
export class AltDynamicDialogModule { }
