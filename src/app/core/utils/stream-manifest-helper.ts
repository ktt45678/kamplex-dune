import { HlsSegmentGroup, StreamManifest } from '../interfaces/video-player';

export function convertToM3U8(manifest: StreamManifest, baseUrl: string) {
  ...
  for (let i = 0; i < manifest.audioTracks.length; i++) {
    ...
    masterStr += `#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID="${track.group}",NAME="${track.name}",DEFAULT=${defaultAudio},`;
    masterStr += `AUTOSELECT=${autoSelectAudio},LANGUAGE="${track.language}",CHANNELS="${track.channels}",`;
  ...
  for (let i = 0; i < manifest.videoTracks.length; i++) {
    ...
    masterStr += `CODECS="${track.codecs}",AUDIO="${defaultAudioGroup}"\n`;
